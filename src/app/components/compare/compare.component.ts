...
export class CompareComponent implements OnInit {
  ...
  categorieSocioProfessionnelleOptions = [
    ...
    { label: 'Agriculteurs exploitants', value: 'AGRICULTEURS_EXPLOITANTS' },
    { label: 'Retraités', value: 'RETRAITES' },
    { label: 'Professions médicales et para médicales Non Salariés', value: 'PROFESSIONS_MEDICALES_NON_SALARIES' },
    { label: 'Sans profession', value: 'PERSONNES_SANS_ACTIVITE_PROFESSIONNELLE' },
    { label: 'Professions médicales et para médicales Salariés', value: 'PROFESSIONS_MEDICALES_SALARIES' },
    { label: 'Mandataire social', value: 'MANDATAIRE_SOCIAL' },
    { label: 'Cadres', value: 'CADRES' },
    { label: 'Employés agents de maîtrise', value: 'EMPLOYES_AGENTS_DE_MAITRISE' },
    { label: 'Ouvriers', value: 'OUVRIERS' },
    { label: 'Cadres et employés de la fonction publique', value: 'CADRES_ET_EMPLOYES_DE_LA_FONCTION_PUBLIQUE' }
  ];
  ...
  private fetchApiviaPrices(): void {
    ...
    apiviaProducts.forEach(result => {
        ...
        if (formulaNumber !== '*' && (parseInt(formulaNumber) < 1 || parseInt(formulaNumber) > 7)) {
          ...
          console.log(`🔍 DEBUG: Formule originale extraite: "${formulaNumber}" depuis "${result.formule}"`);
          formulaNumber = '7';
        }
        
        console.log(`🔍 DEBUG APIVIA: Formule finale utilisée: ${formulaNumber} pour ${result.formule}`);
  ...
  private transformFormToQuote(): InsuranceQuoteForm {
    ...
    insuredPersons.push({
      ...
      categorieSocioProfessionnelle: personalInfo.categorieSocioProfessionnelle, // ✅ AJOUT MANQUANT
  ...
  private normalizeUtwinProductName(name: string): string {
    ...
    // APIVIA LOGIC
      ...
      if (formulaNumber !== '*' && (parseInt(formulaNumber) < 1 || parseInt(formulaNumber) > 7)) {
        console.warn(`Formule Apivia invalide: ${formulaNumber}. Utilisation de la formule 7 par défaut.`);
        console.log(`🔍 DEBUG: Formule originale extraite: "${formulaNumber}" depuis "${formule}"`);
        formulaNumber = '7';
      }
      
      console.log(`🔍 DEBUG APIVIA (onFormuleSelect): Formule finale utilisée: ${formulaNumber} pour ${formule}`);
